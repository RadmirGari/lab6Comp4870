...
[HttpTrigger(AuthorizationLevel.Anonymous, "get", Route = "students")] HttpRequestData req)
    ...
    public HttpResponseData GetStudentBySchool(
        ...
        _logger.LogInformation("C# HTTP Get trigger function getting students in school");
        var filteredStudents = _context.Students.Where(s => s.School == school).ToArray();

        var response = req.CreateResponse(HttpStatusCode.OK);
        response.Headers.Add("Content-Type", "application/json");

        response.WriteStringAsync(JsonConvert.SerializeObject(filteredStudents));


        return response;
    }

    [Function("GetCountOfSchool")]
    public HttpResponseData getCountOfSchool(
        [HttpTrigger(AuthorizationLevel.Anonymous, "get", Route =  "students/count-by-school")] HttpRequestData req
    ){
        _logger.LogInformation("C# HTTP get trigger function getting count.");

        var countBySchool = _context.Students
        ...
        response.WriteStringAsync(JsonConvert.SerializeObject(countBySchool));
